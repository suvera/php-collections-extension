...
$time = microtime(true);
$obj = new StdSuperMap(TYPE_SCALAR_INT, 1000000);
for ($i =0; $i < 1000000; $i++) {
    $obj->push('key'.$i, $i);
}
unset($obj);
echo 'StdSuperMap<String, Int> creation Time: '. round(microtime(true) - $time, 8) . " secs\n";
...
echo 'search on StdSuperMap<String, Int> Time: '. round(microtime(true) - $time, 8) . " secs ($pos)\n";
unset($obj);
...
$time = microtime(true);
$obj = new StdSuperMap(TYPE_COMPLEX_OBJECT, 1000000);
for ($i =0; $i < 1000000; $i++) {
    $obj->push('key'.$i, new ABC($i));
}
unset($obj);
echo 'StdSuperMap<String, Object> creation Time: '. round(microtime(true) - $time, 8) . " secs\n";
...
echo 'Access element in StdSuperMap<String, Int> Time: '. round(microtime(true) - $time, 40) . " secs\n";
...
$time = microtime(true);
$keyObk = $obj->keys();
echo 'getKeys StdSuperMap<String, Int> Time: '. round(microtime(true) - $time, 8) . " secs\n";
...
echo 'search on StdSuperMap<String, Object> Time: '. round(microtime(true) - $time, 8) . " secs ($pos)\n";
$time = microtime(true);
$pos = $obj->search(new ABC(333));
echo 'search on StdSuperMap<String, Object>(value unknown) Time: '. round(microtime(true) - $time, 8) . " secs ($pos)\n";
unset($obj);
